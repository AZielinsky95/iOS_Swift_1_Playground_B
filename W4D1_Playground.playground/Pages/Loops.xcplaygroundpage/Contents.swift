 ...
 */
//2...5  loops from 2 to 5  - 4 times
//2..<5 loops from 2 to 4 - 3 times
for x in 2..<5
{
    print("index \(x)")
}
...
{
    if dict[numberArray[num]] != nil
    {
        dict[numberArray[num]] = dict[numberArray[num]]! + 1
    }
    else
    {
        dict[numberArray[num]] = 1;
    }
}

for item in dict.keys
{
   print("\(item): Occurs \(dict[item]) times")
}
